...
auth = os.environ.get("AUTH_EXANTE_DEMO")
headers={'Accept':'application/x-json-stream'}
r = requests.get('https://api-demo.exante.eu/md/3.0/feed/trades/ETH.USD',stream=True, headers=headers, auth=auth)
    ...
    def generate_data():
        ...
        for i in streamExante:
            ...
            if "price" in i: 
                ...
                json_data = json.dumps({'time': i['timestamp'], 'value': float(i['price'])})
